...
#Read in the Component csv file
Validmu <- read.csv(fname_validmu)
...
#---------------------------TEMP TEST OF HYDRIC RATING
Query_HYDRIC = data.frame(sqldf("SELECT CO_component.mukey, CO_component.compname, CO_component.comppct_r, CO_component.hydricrating, CO_component.majcompflag
FROM Query_INTERM_Get_Major_Component LEFT JOIN CO_component ON Query_INTERM_Get_Major_Component.mukey = CO_component.mukey AND Query_INTERM_Get_Major_Component.MaxOfcomppct_r = CO_component.comppct_r
WHERE CO_component.hydricrating='Yes'"))
#-------------------------

#---------------------------TEMP TEST OF Drainage Class
Query_HYDRIC = data.frame(sqldf("SELECT CO_component.mukey, CO_component.compname, CO_component.comppct_r, CO_component.drainagecl, CO_component.hydricrating, CO_component.majcompflag
FROM Query_INTERM_Get_Major_Component LEFT JOIN CO_component ON Query_INTERM_Get_Major_Component.mukey = CO_component.mukey AND Query_INTERM_Get_Major_Component.MaxOfcomppct_r = CO_component.comppct_r
WHERE CO_component.drainagecl='Very poorly drained' OR CO_component.drainagecl='Poorly drained' "))
#-------------------------
...
#Create Dataframe using SQL For Query_Weld_1k_Union_Valid
Query_Weld_1k_Union_Valid = data.frame(sqldf("SELECT Weld_1k_Union.* 
FROM Weld_1k_Union INNER JOIN Validmu ON Weld_1k_Union.MUKEY = Validmu.mukey"))
